            ...
            <div class="row pt-2">
                <div class="col ps-4">
                    ...
                    <p class="text-primary">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> O período para envio das notas está aberto.
                    </p>
                    @endif
                    <p class="text-primary">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> O envio final das notas deve ser feito apenas uma vez por semestre, enquanto o período de envio estiver aberto.
                    </p>
                    @if ($final_marks_submitted)
                    <p class="text-success">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> Notas enviadas com sucesso.
                    </p>
                    @endif
                    <h3><i class="bi bi-diagram-2"></i> Turma #{{request()->query('class_name')}}, Seção #{{request()->query('section_name')}}</h3>
                    <h3><i class="bi bi-compass"></i> Curso: {{request()->query('course_name')}}</h3>
                    @if (!$final_marks_submitted && count($exams) > 0 && $academic_setting['marks_submission_status'] == "on")
                        <div class="col-12 col-md-3 mt-3">
                            <a type="button" href="{{route('course.final.mark.submit.show', ['class_id' => $class_id, 'class_name' => request()->query('class_name'), 'section_id' => $section_id, 'section_name' => request()->query('section_name'), 'course_id' => $course_id, 'course_name' => request()->query('course_name'), 'semester_id' => $semester_id])}}" class="btn btn-outline-primary w-100" onclick="return confirm('Tem certeza que deseja enviar as notas finais?')"><i class="bi bi-check2"></i> Enviar Notas Finais</a>
                        </div>
                    @endif
                    <form action="{{route('course.mark.store')}}" method="POST">
                        ...
                        <div class="row mt-3">
                                    ...
                                    <table class="table table-hover align-middle">
                                        <thead>
                                            ...
                                            @isset($exams)
                                                ...
                                                <th scope="col"><a href="{{route('exam.rule.show', ['exam_id' => $exam->id])}}" data-bs-toggle="tooltip" data-bs-placement="top" title="Ver regras da prova {{$exam->exam_name}}">{{$exam->exam_name}}</a></th>
                                                @endforeach
                                            @endisset
                                            </tr>
                                        </thead>
                            ...
                            <div class="col-12 col-md-3 mt-3">
                                <button type="submit" class="btn btn-outline-primary w-100"><i class="bi bi-check2"></i> Salvar</button>
                            </div>
                            @else
                                @if($final_marks_submitted)
                                <div class="col-12 col-md-5 mt-3">
                                    <p class="text-success">
                                        <i class="bi bi-exclamation-diamond-fill me-2"></i> Você enviou as Notas Finais <i class="bi bi-stars"></i>.
                                    </p>
                                </div>
                                @else
                                <div class="col-12 col-md-5 mt-3">
